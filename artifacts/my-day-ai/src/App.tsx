...
function Me({ app }: { app: ReturnType<typeof useAppState> }) {
  ...
  useEffect(() => {
    ...
    const error = new URLSearchParams(hashQuery).get('error');
    if (error === 'google_auth_failed') setGoogleError('Google could not finish the connection. Check that this app URL is listed as an authorized redirect URI in your Google OAuth client, then try again.');
    if (error === 'google_denied') setGoogleError('Google access was cancelled. You can try connecting again whenever you are ready.');
  }, []);
