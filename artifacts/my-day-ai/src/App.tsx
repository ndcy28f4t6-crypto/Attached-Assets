...
function Plan({ app }: { app: ReturnType<typeof useAppState> }) {
  ...
  const todayInWeek = weekDays.find(d => d.getTime() === todayMidnight.getTime());
  ...
  const eventsDate = localDateStr(todayInWeek ?? weekStart);
  ...
  const eventsDateLabel = eventsDate === today ? 'Today' : new Date(eventsDate + 'T00:00:00').toLocaleDateString('default', { month: 'short', day: 'numeric' });
